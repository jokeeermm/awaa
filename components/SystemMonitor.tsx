...
import { Cpu, HardDrive, Wifi, Zap, Activity, Server, Database, Network, AlertTriangle, CheckCircle } from "lucide-react";
...
export function SystemMonitor({ className }: SystemMonitorProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'optimal': return 'bg-green-900/30 text-green-400 border-green-400/30';
      case 'degraded': return 'bg-yellow-900/30 text-yellow-400 border-yellow-400/30';
      case 'offline': return 'bg-red-900/30 text-red-400 border-red-400/30';
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm ${metrics.cpu.temperature > 60 ? 'text-red-400' : metrics.cpu.temperature > 50 ? 'text-yellow-400' : 'text-green-400'}`}>
                ...
                <div className="text-xs text-slate-500">
                  Cores: {metrics.cpu.cores} | Architecture: x64
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm text-white">{metrics.memory.used.toFixed(1)} / {metrics.memory.total} GB</span>
                </div>
                <Progress value={metrics.memory.usage} className="h-2 bg-slate-700" />
                
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-400">Available</span>
                  <span className="text-sm text-green-400">{(metrics.memory.total - metrics.memory.used).toFixed(1)} GB</span>
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm ${metrics.network.latency < 50 ? 'text-green-400' : metrics.network.latency < 100 ? 'text-yellow-400' : 'text-red-400'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="text-xs text-slate-500">
                  Learning Rate: {metrics.ai.learningRate.toFixed(4)}
                </div>
              </div>
            </div>
          </div>
            ...
            {metrics.cpu.usage > 80 && (
              <div className="flex items-center gap-3 p-3 bg-yellow-900/20 rounded-lg border border-yellow-700/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">CPU usage is at {metrics.cpu.usage.toFixed(1)}%. Consider scaling resources.</p>
                </div>
              </div>
            )}
            ...
            {metrics.memory.usage > 75 && (
              <div className="flex items-center gap-3 p-3 bg-orange-900/20 rounded-lg border border-orange-700/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Memory usage is at {metrics.memory.usage.toFixed(1)}%. Monitor for potential issues.</p>
                </div>
              </div>
            )}
            ...
            {metrics.network.latency > 80 && (
              <div className="flex items-center gap-3 p-3 bg-red-900/20 rounded-lg border border-red-700/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Network latency is {metrics.network.latency.toFixed(0)}ms. Check network connectivity.</p>
                </div>
              </div>
            )}
