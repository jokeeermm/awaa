...
export function OrderTracking({ onBack }: OrderTrackingProps) {
  ...
  const getStatusIcon = (status: TrackingOrder['status']) => {
    switch (status) {
      case 'analyzing': return <Bot className="w-4 h-4 text-blue-400" />;
      case 'verifying': return <Clock className="w-4 h-4 text-yellow-400" />;
      case 'transferring': return <Activity className="w-4 h-4 text-purple-400 animate-pulse" />;
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-3">
              <div className="flex-1">
                <Label htmlFor="trackingId" className="text-slate-300">AI Agent ID</Label>
                <Input
                  id="trackingId"
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Badge className="bg-green-900/30 text-green-400 border-green-400/30 text-xs">
                          Excellent
                        </Badge>
                      </div>
                    </div>
                  ...
                  {searchResult.status !== 'completed' && searchResult.status !== 'failed' && (
                    <div className="flex items-start gap-4 p-4 bg-blue-900/20 rounded-lg border border-blue-700/30">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-slate-400 text-sm">AI neural network is actively processing your request...</p>
                      </div>
                    </div>
                  )}
                </div>
