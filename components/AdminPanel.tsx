...
import { ArrowLeft, Settings, Edit, DollarSign, FileText, User, Calendar, Bot, Shield, Activity, Cpu, Database, Terminal } from "lucide-react";
...
export function AdminPanel({ onBack }: AdminPanelProps) {
  ...
  const getStatusColor = (status: Order['status']) => {
    switch (status) {
      case 'analyzing': return 'bg-blue-900/30 text-blue-400 border-blue-400/30';
      case 'verifying': return 'bg-yellow-900/30 text-yellow-400 border-yellow-400/30';
      case 'transferring': return 'bg-purple-900/30 text-purple-400 border-purple-400/30';
      case 'completed': return 'bg-green-900/30 text-green-400 border-green-400/30';
      case 'failed': return 'bg-red-900/30 text-red-400 border-red-400/30';
  ...
  const updateOrderPrice = (orderId: string, newAmount: number) => {
    setOrders(orders.map(order => 
      order.id === orderId ? { ...order, amount: newAmount } : order
    ));
    setNewPrice('');
  };
        ...
        <Card className="w-full max-w-md bg-slate-800/50 border-slate-700">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-slate-400">
              Secure neural network administration panel
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="username" className="text-slate-300">Administrator ID</Label>
              <Input
                id="username"
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl text-white">${orders.reduce((sum, order) => sum + order.amount, 0).toFixed(2)}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl text-white">{orders.filter(o => ['analyzing', 'verifying', 'transferring'].includes(o.status)).length}</p>
                ...
                <TableBody>
                  {orders.map((order) => (
                    <TableRow key={order.id} className="border-slate-700 hover:bg-slate-700/30">
                      ...
                      <TableCell>
                        {order.aiScore && (
                          <Badge className={`${order.aiScore > 95 ? 'bg-green-900/30 text-green-400 border-green-400/30' : 
                                         order.aiScore > 90 ? 'bg-blue-900/30 text-blue-400 border-blue-400/30' : 
                      ...
                      <TableCell>
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button variant="ghost" size="sm" onClick={() => setSelectedOrder(order)} className="hover:bg-slate-700">
                              ...
                              <div className="space-y-6">
                                <div className="space-y-3">
                                  ...
                                  >
                                    ...
                                    <SelectContent className="bg-slate-800 border-slate-700">
                                      <SelectItem value="analyzing" className="text-white">🧠 Analyzing</SelectItem>
                                      <SelectItem value="verifying" className="text-white">🔍 Verifying Payment</SelectItem>
                                      <SelectItem value="transferring" className="text-white">⚡ Transferring Members</SelectItem>
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    />
                                    <Button 
                                      onClick={() => updateOrderPrice(selectedOrder.id, parseFloat(newPrice))}
                                      disabled={!newPrice}
